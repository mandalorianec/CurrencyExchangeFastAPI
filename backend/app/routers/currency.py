...
from app.schemas import CurrencyResponse, CurrencyCode, CurrencySchema, ApiErrorSchema
...
async def add_new_currency(currency_service: CurrencyServiceDep, currency: Annotated[CurrencySchema, Form()]):
    created_currency = await currency_service.add_currency(currency)
    return created_currency


@currency_router.get('/currency/{code}', response_model=CurrencyResponse,
                     responses={
