...
async def test_convert_usd_rub(
    exchange_service: ExchangeService, amount: Decimal, rub_currency, usd_currency, exchange_rate_usd_rub
) -> None:
...
@pytest.mark.anyio
@pytest.mark.parametrize("amount", [(Decimal(24))])
async def test_convert_rub_usd(
    exchange_service: ExchangeService, amount: Decimal, rub_currency, usd_currency, exchange_rate_usd_rub
) -> None:
...
@pytest.mark.anyio
async def test_convert_eur_rub(
    exchange_service: ExchangeService, eur_currency, usd_currency, exchange_rate_usd_rub, exchange_rate_usd_eur
) -> None:
