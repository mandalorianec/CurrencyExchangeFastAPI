...
from app.dependencies import ExchangeRateServiceDep, CurrencyServiceDep, _divide_codepair
from app.exceptions import CurrencyNotFoundError
from app.schemas import ExchangeRateResponse, ExchangeRateSchema, InputDecimal, ApiErrorSchema

exchange_rate_router = APIRouter(tags=['Операции с обменным курсами'])


@exchange_rate_router.get("/exchangeRates", response_model=list[ExchangeRateResponse],
...
@exchange_rate_router.post("/exchangeRates",
...
async def add_new_exchangerate(exchangerate_service: ExchangeRateServiceDep,
    ...
    try:
        ...
        raise HTTPException(status_code=e.code, detail="Одна (или обе) валюта из валютной пары не существует в БД")
...
@exchange_rate_router.get("/exchangeRate/{codepair}", response_model=ExchangeRateResponse,
                          responses={
                              ...
                              500: {"model": ApiErrorSchema, "description": "База данных недоступна"}})
async def get_exchangerate_by_codepair(exchangerate_service: ExchangeRateServiceDep,
...
@exchange_rate_router.patch("/exchangeRate/{codepair}", response_model=ExchangeRateResponse,
                            responses={
                                400: {"model": ApiErrorSchema, "description": "Отсутствует нужное поле формы"},
                                404: {"model": ApiErrorSchema, "description": "Валютная пара отсутствует в базе данных"},
                                500: {"model": ApiErrorSchema, "description": "База данных недоступна"},
                            })
async def change_exchangerate_by_codepair(exchangerate_service: ExchangeRateServiceDep,
                                          codes: Annotated[tuple[str, str], Depends(_divide_codepair)],
