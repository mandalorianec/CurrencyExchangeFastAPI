...
class ExchangeRateRepository:
    ...
    async def get_all(self) -> list[ExchangeRate]:
        exchangerates = await self.session.execute(select(ExchangeRate).options(
    ...
    async def get_exchangerate_by_codepair(self, base_code: str, target_code: str) -> ExchangeRate:
        ...
        exchangerate = await self.session.execute(select(ExchangeRate)
                                                  .join(base_aliase, ExchangeRate.base_currency_id == base_aliase.id)
                                                  .join(target_aliase,
                                                        ExchangeRate.target_currency_id == target_aliase.id)
                                                  .options(
            ...
            contains_eager(ExchangeRate.target_currency, alias=target_aliase))
                                                  .filter(base_aliase.code == base_code,
                                                          target_aliase.code == target_code)
                                                  )
